...
bool Entity::interactWithX(Entity* object){
    ...
    interactRight =false;
    //for some reason the image on screen get stretch to double, so i have to times 3 of height, and double the width
    float obot = object->y - (object->height/2)  * object->scale;
    float otop = object->y + (object->height/2)  * object->scale;
    float oleft = object->x - (object->width/2)  * object->scale;
    float oright = object->x + (object->width/2)  * object->scale;
